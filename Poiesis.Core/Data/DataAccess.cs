...
namespace Poiesis.Core.Base
{
    ...
    public static class DataAccess
    {
        ...
        public static bool TransferSchema(string sourceConnectionString, string newConnectionString, ILogger logger)
        {
            try
            {
                ServerConnection sourceConnection = new ServerConnection(new SqlConnection(sourceConnectionString));
                Server sourceServer = new Server(sourceConnection);

                ServerConnection destinationConnection = new ServerConnection(new SqlConnection(newConnectionString));
        ...
        public static bool TransferData(string sourceConnectionString, string newConnectionString, ILogger logger)
        {
            try
            {
                using (SqlBulkCopy bulkCopy = new SqlBulkCopy(newConnectionString) { BatchSize = 500, NotifyAfter = 1000 })
                {
                    ServerConnection sourceConnection = new ServerConnection(new SqlConnection(sourceConnectionString));
                    Server sourceServer = new Server(sourceConnection);
